...
def join_words(words: list, separator: str) -> str:
    ...
    result = ""
    for word in words:
        # raise AssertionError if list item is not a string
        assert isinstance(word, str), "list must only contain strings"
        # if the result is empty, we don't add the separator
        if result == "":
            result = word
        else:
